...
export class Login implements OnInit, OnDestroy {
  ...
  async onSignUp() { const { email, password, name } = this.form.value; await this.auth.signUp(email ?? '', password ?? '', name ?? undefined); this.router.navigateByUrl('/landing'); }
  async onSignIn() { if (this.form.invalid) return; const { email, password } = this.form.value; await this.auth.signIn(email!, password!); this.router.navigateByUrl('/landing'); }
  ...
  async onGoogle() { try { await this.auth.signInWithGoogle(); this.router.navigateByUrl('/landing'); } catch {} }
